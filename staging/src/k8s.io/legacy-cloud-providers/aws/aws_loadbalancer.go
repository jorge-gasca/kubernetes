		...
		// sync mappings
		{
			...
			for _, mapping := range mappings {
				...
				if listener, ok := actual[frontendPort]; ok {
					...
					healhCheckOk := aws.StringValue(targetGroup.HealthCheckProtocol) == elbv2.ProtocolEnumTcp && aws.StringValue(targetGroup.HealthCheckProtocol) != mapping.HealthCheckProtocol
					if !ok || aws.StringValue(targetGroup.Protocol) != mapping.TrafficProtocol || !healhCheckOk {
